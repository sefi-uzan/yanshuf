...
export function registerTools(server: McpServer, client: YanshufApiClient): void {
  ...
  server.registerTool(
    ...
    {
      description:
        'Search captured request summaries (latest first, max 100). Use before yanshuf_get_capture to find IDs.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Block until a new capture arrives or timeout. Use after triggering traffic or yanshuf_send_request.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Send an HTTP request through Yanshuf proxy. Optionally pass captureId to replay a captured request.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Create or update a mock response rule. Optionally pass captureId to bootstrap from a capture.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Create or update an intercept rule (rewrite or breakpoint). Optionally pass captureId to bootstrap.',
      inputSchema: {
        id: z.string().optional(),
        captureId: z.string().optional(),
        name: z.string().optional(),
  ...
  server.registerTool(
    ...
    async ({ id, headers, body, status }) =>
      textResult(await client.continueBreakpoint(id, { headers, body, status })),
  );
